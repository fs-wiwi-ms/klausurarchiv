defmodule KlausurarchivWeb.ConfirmedEmail do
  ...
  import Phoenix.Controller, only: [put_flash: 3, render: 2, put_view: 2, redirect: 2]
  ...
  def call(conn, _) do
    with user_id when not is_nil(user_id) <- get_session(conn, :user_id),
        ...
        if !user.email_confirmed do
          ...
          |> redirect(
            to: KlausurarchivWeb.Router.Helpers.account_confirmation_path(conn, :not_confirmed)
          )
          |> halt()
        else
          conn
